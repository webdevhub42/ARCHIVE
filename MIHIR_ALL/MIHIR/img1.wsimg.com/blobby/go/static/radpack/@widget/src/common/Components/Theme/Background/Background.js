...
import { uniqueId, isEqual, isFunction } from "lodash";
import { utils, UX2 } from "@wsb/guac-widget-core";
import {
  CDN_REGEX,
  addImageApiArgs,
...
export class Background extends UX2.utils.createElement("Background") {
  ...
  constructor() {
    ...
    const { backgroundImage, treatmentData, imageData = {} } = this.props;
    const { widgetType, widgetPreset } = this;
    this.state = {
      id: uniqueId("guacBg"),
      backgroundImage: backgroundImage || generateBackgroundUrl(imageData),
      isResponsive: CDN_REGEX.test(
        backgroundImage || generateBackgroundUrl(imageData)
      ),
      defaultFilter:
        !imageData.filter &&
        this.theme &&
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    const { backgroundImage, treatmentData, defaultFilter } = prevState;
    const { treatmentData: nextTreatmentData, imageData = {} } = nextProps;

    const nextBackgroundImage = nextProps.backgroundImage
      ? addImageApiArgs(
          nextProps.backgroundImage,
          utils.generateImageFilterUrlParam(imageData.filter || defaultFilter)
        )
      : imageData &&
        generateBackgroundUrl({
          ...
          ...imageData,
          ...(nextProps.backgroundImage &&
            nextProps.backgroundImage.indexOf("fx-bl=s") > -1 && {
    ...
    const state = {
      ...
      ...(nextTreatmentData &&
        !isEqual(treatmentData, nextTreatmentData) &&
        nextTreatmentData),
    };
  ...
  onRender(imageUrl) {
    if (this.mounted) {
      const { treatmentData } = this.props;
...
export class BackgroundResponsive extends UX2.utils.createElement(
  "Background",
  "Responsive"
) {
  constructor(props) {
