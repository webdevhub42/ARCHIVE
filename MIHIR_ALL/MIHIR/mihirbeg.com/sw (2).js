...
const pageUrls = JSON.parse(
  '["/","/live-sound","/listen","/debut-album-getting-there","/about","/watch"]'
);
...
const networkOnlyUrls = JSON.parse(
  '["https://api.ola.godaddy.com","https://03387d70-a2b8-40e7-acae-dc6a382237e9.onlinestore.godaddy.com","https://img.secureserver.net/t/1/tl/event","https://img.test-secureserver.net/t/1/tl/event","https://www.google-analytics.com/collect","https://calendar.apps.dev-secureserver.net","https://calendar.apps.test-secureserver.net","https://calendar.apps.secureserver.net"]'
);
const networkOnlyUrlsRegex = JSON.parse(
  '["mihirbeg.com/m/api/.*","mihirbeg.com(?:/.*)?/ola/services/.*","mihirbeg.com/ola/meetings/.*","mihirbeg.com/g/api/.*","securepay.godaddy.com/api/apps/ola/accounts/.*"]'
).map((regexString) => new RegExp(regexString));
const networkThenCacheUrls = JSON.parse(
  '["https://blog.apps.secureserver.net/v1/website/03387d70-a2b8-40e7-acae-dc6a382237e9/feed/post/","https://blog.apps.secureserver.net/v1/website/03387d70-a2b8-40e7-acae-dc6a382237e9/feed"]'
);
const networkThenCacheUrlsRegex = JSON.parse(
  '["mihirbeg.com(?:/.*)?/f/.*"]'
).map((regexString) => new RegExp(regexString));
...
function isNetworkOnlyRequest(url, requestMethod) {
  ...
  if (
    requestMethod !== "GET" ||
    !SW_SUPPORTED_PROTOCOL_REGEX.test(url.protocol)
  ) {
    return true;
  }
  ...
  if (
    networkOnlyUrls.includes(urlOrigin) ||
    networkOnlyUrls.includes(fullUrl)
  ) {
    return true;
  }
...
function isNetworkThenCacheRequest(url, requestMethod) {
  ...
  if (
    networkThenCacheUrls.includes(urlOrigin) ||
    networkThenCacheUrls.includes(fullUrl)
  ) {
    return true;
  }
...
function handleRequests(event) {
  ...
  if (
    isPageRequest(requestURL) ||
    isNetworkThenCacheRequest(requestURL, event.request.method)
  ) {
