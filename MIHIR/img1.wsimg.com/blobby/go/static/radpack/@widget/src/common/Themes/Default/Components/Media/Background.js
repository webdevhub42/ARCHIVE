...
import { getValidUrl, addImageApiArgs } from "../../../../utils/image";
...
export function Background({
  ...
  const _style = {
    ...
    ...style,
    transition:
      (style && style.transition ? style.transition + ", " : "") +
  ...
  if (backgroundImageState) {
    ...
    if (isResponsive) {
      const url =
        responsiveImage ||
        utils.Image.getUrl(
          addImageApiArgs(backgroundImageState, {
            qt: "q:1",
          }),
          {
            x: 50,
      ...
      _style.backgroundImage = `url("${getValidUrl(
        url.replace(/"/g, '\\"')
      )}")`;
    ...
    if (layers.length > 0) {
      ...
      _style.backgroundImage = []
        .concat(layers, [_style.backgroundImage || bg || ""])
        .filter(Boolean)
        .join(", ");
    }
    ...
    if (layers.length) {
      _style.backgroundSize = `${"auto, ".repeat(layers.length)}${
        _style.backgroundSize
      }`;
    }

    if (mobileImage) {
      const mobileBgImage = layers.length
        ? `${layers.join(", ")}, url("${mobileImage.replace(/"/g, '\\"')}")`
