...
} from '../../../common/Themes/Default/Dials/Colors/Default/colors';
...
const resolveColor = (colorConfig, defaultColor) => {
    const colorComparison = colorConfig.filter(({
        precheck
    }) => precheck);
    ...
    return (!colorLightnessSimilarity.apply(null, contrastingColors) && priorityResolution[0]) || priorityResolution[1];
};
...
class Theme21 extends Default {
    ...
    static excludedProps = without(Default.excludedProps, 'address').concat('tagline2')
    ...
    static getWidgetDefaultProps(widgetType, widgetPreset) {
        return {
            ...
            ...((widgetType === 'HEADER' || !widgetsWithCenteredText.includes(widgetPreset)) && {
    ...
    constructor() {
        ...
        this.mappedValues = {
            ...
            borderColorHighlight: () => {
                const [background, primary, neutral, accent] = utils.getDial(this, ['background', 'primary', 'neutral', 'accent']);
                const colorConfig = [{
    ...
    Hero(props) {
        ...
        return super.Hero(Object.assign(props, {
            style: newStyle
        }));
    }
    ...
    ContentCard(props) {
        ...
        const overrides = widgetsWithCenteredText.indexOf(widgetPreset) === -1 ? {
            alignment: 'left'
        } : {};
    ...
    FeaturedText(props) {
        return super.FeaturedText(this.merge({
            style: {
                fontStyle: hasFontSet(this.base.fonts, 'old-standard-tt') ? 'italic' : 'normal !important'
            },
            typography: 'BodyBeta'
        }, props));
    }
    ...
    Button(props) {
        ...
        const {
            fill = this.getButtonDefaults().fill, shadow = this.getButtonDefaults().shadow
        } = props;
    ...
    Heading(props) {
        ...
        const footerStyles = this.base.widgetType === 'FOOTER' ? {
            level: '5',
            style: {
                fontSize: '14px'
            }
        ...
        return super.Heading(this.merge(footerStyles, {
