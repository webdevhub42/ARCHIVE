...
export default class GridCell extends utils.createElement('Grid', 'Cell') {
    ...
    static applySizeStyles(style = {}, props = {}) {
        ...
        if (sm !== xs || gridSm !== gridXs) {
            const value = sm > 0 || gridSm > 0 ?
                this.getPercent(sm, gridSm) :
                '0%';
            if (value !== basis) {
                basis = value;
        ...
        if (md !== sm || gridMd !== gridSm) {
            const value = md > 0 || gridMd > 0 ?
                this.getPercent(md, gridMd) :
                '0%';
            if (value !== basis) {
                basis = value;
        ...
        if (lg !== md || gridLg !== gridMd) {
            const value = lg > 0 || gridLg > 0 ?
                this.getPercent(lg, gridLg) :
                '0%';
            if (value !== basis) {
                basis = value;
