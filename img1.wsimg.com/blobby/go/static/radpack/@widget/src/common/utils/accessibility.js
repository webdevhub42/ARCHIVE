...
const hsvDistance = (hsv1, hsv2) => Math.pow(hsv1.saturation - hsv2.saturation, 2) + Math.pow(hsv1.value - hsv2.value, 2);
...
function findThreshold({
    ...
    for (let value = initialValue; value <= 100; value++) {
        const contrastRatio = Color.contrastRatio(backgroundRgb, Color.hsvToRgb(hue, saturation, value));
        if (isCurrentValueAccessible && contrastRatio < (threshold + CONTRAST_ERROR_MARGIN)) return value - 1;
        else if (!isCurrentValueAccessible && contrastRatio >= (threshold + CONTRAST_ERROR_MARGIN)) return value;
    }
}
...
export function getNearestAccessibleColors(contrastThreshold, background, foreground, count = 1) {
    if (Color.contrastRatio(foreground, background) >= (contrastThreshold + CONTRAST_ERROR_MARGIN)) return [foreground]; // already accessible

    const [hue, saturation, value] = foreground.toHsv();
    const curve = getAccessibilityCurve(contrastThreshold, background, foreground, hue);
    const sortedDistances = curve.map((v, i) => {
        return {
            saturation: i,
            value: v,
